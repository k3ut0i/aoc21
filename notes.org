...
* DONE Day 14
  Solved the first part using a direct simulation. The second part requires more as the it is taking exponential time and memory. There are finite patterns here. Hence there will be big parts of the chain just repeating in time and space.
  ...
  Ends of the polymer chain are proving a bit of problem for boundry conditions. I kept track of them and when took them into account when counting the characters.
